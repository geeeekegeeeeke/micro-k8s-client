...
func (u *DockerService) PageNetwork(req dto.SearchWithPage) (int64, interface{}, error) {
	//TODO implement me
	panic("implement me")
}

func (u *DockerService) PageVolume(req dto.SearchWithPage) (int64, interface{}, error) {
	//TODO implement me
	panic("implement me")
}
...
func (u *DockerService) CreateCompose(req dto.ComposeCreate) (string, error) {
	//TODO implement me
	panic("implement me")
}

func (u *DockerService) ComposeOperation(req dto.ComposeOperation) error {
	//TODO implement me
	panic("implement me")
}

func (u *DockerService) DeleteNetwork(req dto.BatchDelete) error {
	//TODO implement me
	panic("implement me")
}

func (u *DockerService) CreateNetwork(req dto.NetworkCreate) error {
	//TODO implement me
	panic("implement me")
}

func (u *DockerService) DeleteVolume(req dto.BatchDelete) error {
	//TODO implement me
	panic("implement me")
}

func (u *DockerService) CreateVolume(req dto.VolumeCreate) error {
	//TODO implement me
	panic("implement me")
}

func (u *DockerService) TestCompose(req dto.ComposeCreate) (bool, error) {
	//TODO implement me
	panic("implement me")
}

func (u *DockerService) ComposeUpdate(req dto.ComposeUpdate) error {
	//TODO implement me
	panic("implement me")
}
...
func (u *DockerService) LoadContainerLogs(req dto.OperationWithNameAndType) string {
	filePath := ""
	switch req.Type {
	case "image-pull", "image-push", "image-build", "compose-create":
		filePath = path.Join("", fmt.Sprintf("docker_logs/%s", req.Name))
		//case "compose-detail":
		//	client, err := docker.NewDockerClient()
		//	if err != nil {
		//		return ""
...
func (u *DockerService) List() ([]string, error) {
	client, err := docker.NewDockerClient()
	if err != nil {
		return nil, err
	}
...
func (u *DockerService) Page(req dto.PageContainer) (int64, interface{}, error) {
	var (
	...
	total, start, end := len(list), (req.PageNo-1)*req.PageSize, req.PageNo*req.PageSize
	if start > total {
...
func (u *DockerService) Inspect(req dto.InspectReq) (string, error) {
	client, err := docker.NewDockerClient()
	if err != nil {
		return "", err
	}
...
func (u *DockerService) Prune(req dto.ContainerPrune) (dto.ContainerPruneReport, error) {
...
func (u *DockerService) LoadResouceLimit() (*dto.ResourceLimit, error) {
...
func (u *DockerService) ContainerCreate(req dto.ContainerOperate) error {
	client, err := docker.NewDockerClient()
	if err != nil {
		return err
	}
	...
	log.Println("create container %s successful! now check if the container is started and delete the container information if it is not.", req.Name)
...
func (u *DockerService) ContainerInfo(req dto.OperationWithName) (*dto.ContainerOperate, error) {
	client, err := docker.NewDockerClient()
	if err != nil {
		return nil, err
	}
...
func (u *DockerService) ContainerUpdate(req dto.ContainerOperate) error {
	client, err := docker.NewDockerClient()
	if err != nil {
		return err
	}
	...
	log.Println("new container info %s has been update, now start to recreate", req.Name)
	container, err := client.ContainerCreate(ctx, config, hostConf, &network.NetworkingConfig{}, &v1.Platform{}, req.Name)
	if err != nil {
		return fmt.Errorf("recreate contianer failed, err: %v", err)
	}
	log.Println("update container %s successful! now check if the container is started.", req.Name)
	if err := client.ContainerStart(ctx, container.ID, types.ContainerStartOptions{}); err != nil {
...
func (u *DockerService) ContainerUpgrade(req dto.ContainerUpgrade) error {
	client, err := docker.NewDockerClient()
	if err != nil {
		return err
	}
...
func (u *DockerService) ContainerOperation(req dto.ContainerOperation) error {
	var err error
	ctx := context.Background()
	client, err := docker.NewDockerClient()
	if err != nil {
	...
	log.Println("start container %s operation %s", req.Name, req.Operation)
...
func (u *DockerService) ContainerLogClean(req dto.OperationWithName) error {
	client, err := docker.NewDockerClient()
	if err != nil {
		return err
	}
...
func (u *DockerService) ContainerLogs(wsConn *websocket.Conn, container, since, tail string, follow bool) error {
	...
	for {
		...
		if err != nil {
			...
			log.Println("read bytes from container log failed, err: %v", err)
			continue
		}
		if err = wsConn.WriteMessage(websocket.TextMessage, buffer[:n]); err != nil {
			log.Println("send message with container log to ws failed, err: %v", err)
			break
		}
	}
	return nil
...
func (u *DockerService) ContainerStats(id string) (*dto.ContainterStats, error) {
	client, err := docker.NewDockerClient()
	if err != nil {
		return nil, err
	}
