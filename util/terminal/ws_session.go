	...
	for {
		...
		default:
			...
			case WsMsgResize:
				if msgObj.Cols > 0 && msgObj.Rows > 0 {
					if err := sws.session.WindowChange(msgObj.Rows, msgObj.Cols); err != nil {
						log.Println("ssh pty change windows size failed, err: %v", err)
					}
				}
			case WsMsgCmd:
				decodeBytes, err := base64.StdEncoding.DecodeString(msgObj.Data)
				if err != nil {
					log.Println("websock cmd string base64 decoding failed, err: %v", err)
				}
			...
			case WsMsgHeartbeat:
				...
				if err != nil {
					log.Println("ssh sending heartbeat to webSocket failed, err: %v", err)
				}
			}
		}
	}
...
func (sws *LogicSshWsSession) sendWebsocketInputCommandToSshSessionStdinPipe(cmdBytes []byte) {
	if _, err := sws.stdinPipe.Write(cmdBytes); err != nil {
		log.Println("ws cmd bytes write to ssh.stdin pipe failed, err: %v", err)
	}
}
...
func (sws *LogicSshWsSession) sendComboOutput(exitCh chan bool) {
	...
	for {
		...
		case <-tick.C:
			...
			if len(bs) > 0 {
				wsDatamy, err := json.Marshal(WsMsg{
					Type: WsMsgCmd,
					Data: string(bs),
				})
				log.Println(string(wsDatamy))
				wsData, err := json.Marshal(WsMsg{
					Type: WsMsgCmd,
					Data: base64.StdEncoding.EncodeToString(bs),
				})
				if err != nil {
					log.Println("encoding combo output to json failed, err: %v", err)
					continue
				}
				//log.Println(wsData)
				err = wsConn.WriteMessage(websocket.TextMessage, wsData)
				if err != nil {
					log.Println("ssh sending combo output to webSocket failed, err: %v", err)
				}
				_, err = sws.logBuff.Write(bs)
				if err != nil {
					log.Println("combo output to log buffer failed, err: %v", err)
				}
